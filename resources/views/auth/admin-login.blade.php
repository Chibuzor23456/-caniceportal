<x-layouts.bare title="Administrator Login">
    <div class="flex min-h-screen flex-col items-center justify-center px-6 py-12">
        ...
        <div class="w-full max-w-sm rounded-2xl bg-white p-8 shadow-sm">
            <h1 class="text-xl font-bold text-slate-900">Administrator Login</h1>
            <p class="mt-1 text-sm text-slate-500">Full access to clients, quotations, and everything else.</p>
            ...
            <form method="POST" action="{{ route('admin.login.store') }}" class="mt-6 space-y-4">
                ...
                <button type="submit" class="w-full rounded-xl bg-navy px-4 py-2.5 text-sm font-medium text-white hover:bg-slate-800">
                    Log In
                </button>
            </form>
        </div>

        <a href="{{ route('login') }}" class="mt-6 text-xs text-slate-400 hover:text-slate-600">Client login &rarr;</a>
    </div>
</x-layouts.bare>
