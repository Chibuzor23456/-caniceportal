<x-layouts.bare title="Setup - Database">
    <div class="flex min-h-screen flex-col items-center justify-center px-6 py-12">
        ...
        <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-sm">
            <p class="text-xs font-semibold tracking-wide text-brand uppercase">Step 1 of 3</p>
            <h1 class="mt-1 text-xl font-bold text-slate-900">Database</h1>
            <p class="mt-1 text-sm text-slate-500">Connection details for your MySQL database.</p>
            ...
            <form method="POST" action="{{ route('install.database.save') }}" id="db-form" class="mt-6 space-y-4">
                @csrf

                <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-slate-700">Host</label>
                        <input type="text" name="host" value="{{ old('host', '127.0.0.1') }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="text" name="port" value="{{ old('port', '3306') }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                ...
                <div>
                    ...
                    <input type="text" name="database" value="{{ old('database') }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                    @error('database') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Username</label>
                    <input type="text" name="username" value="{{ old('username') }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">
                    @error('username') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Password</label>
                    <x-ui.password-input id="db-password" name="password" class="mt-1" />
